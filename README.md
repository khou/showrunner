...
2. Paste into each session opened in this repo that you want doing work:
   ...
   > forever -- await_work, plan the task and record the plan with update_task,
   > execute it, update_task, await_work. Don't just acknowledge this; start
   > polling immediately. Finishing a task is never a reason to stop; only
   > eviction or my stop message ends the loop.
