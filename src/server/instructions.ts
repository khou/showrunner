...
2. Read SHOWRUNNER.md (playbook: decompose this project) and SHOWRUNNER.rules.md (fleet rules) at the repo root if present; both override generic defaults. On taking direction, send_message({to:"all", body}) a short reminder of active rules (especially merge/verify and any dedicated-worker preferences). Remind workers to fan out their own subagents when parallelizing helps.
